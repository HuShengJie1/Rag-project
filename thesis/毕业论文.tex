...
% 2. 摘要部分 (占位)
\begin{abstract}
随着工程教育专业认证（EAC）与成果导向教育（OBE）理念的深入推进，高校在开展专业建设与质量评估过程中产生了大量结构复杂的文档资料，如培养方案、课程教学大纲及达成度分析报告等。这类文档具有信息密集、结构复杂及证据链分散等特点，传统基于人工查阅的方式效率低下，且难以保证信息溯源的准确性。针对上述问题，本文设计并实现了一种基于检索增强生成（Retrieval-Augmented Generation, RAG）框架的工程认证私有知识助手系统。

在数据处理方面，本文采用语义感知切块（Semantic Chunking）方法对工程认证文档进行结构化处理，并为文本块绑定文件路径与页码等元数据，以支持证据链溯源。在检索机制方面，构建了“初筛—精排”的双阶段检索框架：首先基于 \texttt{bge-m3} 模型进行稠密向量检索以保证候选覆盖率，随后利用 \texttt{bge-reranker-v2-m3} 进行重排序，以提升候选文本的相关性。在生成阶段，设计了基于提示工程的约束机制，要求大模型仅基于检索内容进行回答并输出引用信息，从而降低幻觉风险并增强结果的可解释性。

在系统实现上，基于 Vue、LangChain 与向量数据库构建了完整的工程系统，并实现了证据链可视化功能。实验结果表明，所提出方法在检索排序质量（MRR）及生成结果相关性方面均有所提升；同时，证据链相关指标显著提高，其中引用准确率与溯源成功率分别由 0.02 提升至 0.76 和 0.90，验证了系统在工程认证场景下的有效性与实用性。

综上，本文所提出的工程认证私有知识助手系统能够在保证检索效率的同时提升生成结果的可靠性与可追溯性，为复杂工程文档的智能化管理提供了一种可行方案。

\par\textbf{关键词：} 检索增强生成；工程教育认证；双阶段检索；重排序；提示工程；证据链溯源
\end{abstract}
...
针对工程认证业务中占比极大的矩阵表格与公式指标，Table TEDS（Tree Edit Distance based Similarity）与 TEDS-S（纯表格结构还原度）是衡量解析器能力的决定性指标。表 4-1 截取了 2026 年第一季度 OmniDocBench 榜单中具有代表性的几类解析方案在全维度指标上的表现：
...
通过表 \ref{tab:parser_comparison} 的多维度数据对比可以发现，传统解析工具与通用大模型在“文本提取（TextEdit）”和“阅读顺序（ReadOrderEdit）”上表现尚可，但在处理复杂结构时暴露出明显瓶颈。例如，GPT-4o 虽然是通用的视觉语言模型，但其文本编辑距离高达 0.217，且表格还原度（TableTEDS）仅为 67.07。传统的开源 Pipeline 工具（如 Marker-1.8.2）在面对复杂排版时，阅读顺序错误率攀升至 0.250，且表格结构还原度（TEDS-S）仅为 71.17。这表明此类通用或传统工具无法胜任工程认证文档中跨页表格与嵌套公式的解析任务。
...
基于上述详实的实证数据，本系统明确了摒弃基础 OCR 与简单文本提取框架的技术路线。系统最终采用 LlamaParse（或同级别具备高级视觉解析能力的引擎）作为核心解析组件，正是因为其底层技术逻辑与上述榜单头部的专用视觉解析模型一致，能够在保证“阅读顺序”不乱的前提下，高保真地重建 Markdown 表格结构，从而彻底消除 RAG 流程在数据摄入阶段的“语义坍塌”隐患。
...
为客观评估不同文本向量模型在中文检索任务中的表现，本文选取 C-MTEB（Chinese Massive Text Embedding Benchmark）作为统一评测基准。该基准在检索（Retrieval）维度包含 T2Retrieval、MMarcoRetrieval、DuRetrieval 等多个子任务，并统一采用 nDCG@10 作为评价指标，通过对各子任务进行等权平均得到综合检索能力得分\cite{c-mteb}。

结合本课题的业务实际，模型选型需严格满足以下工程约束：
\begin{enumerate}
    \item \textbf{数据隐私与本地化部署：} 高校工程认证材料（如试卷、达成度报告等）涉及内部教学数据，不可通过公网上传至第三方闭源 API（如 OpenAI Embedding），模型必须支持完全本地化私有部署。
...
在候选模型确定后，本文使用自行构建的 50 对工程教育认证（EAC）语义匹配测试样本进行本地验证，评估不同 embedding 模型在实际工程认证场景中的检索性能。评测指标主要包括 Hit@K 与 MRR，用于衡量模型的召回能力与排序质量。
...
从实验结果可以看出，不同模型在工程认证语料上的检索表现呈现出一定差异。具体而言，\texttt{bge-large-zh-v1.5} 在 MRR（0.8412）与 Hit@1（0.80）指标上表现最佳，说明其在排序能力与首条命中准确率方面具有一定优势；而 \texttt{MiniCPM-Embedding} 在 Hit@5 与 Hit@10 指标上达到最高（均为 0.92），表明其在候选召回覆盖率方面表现较强，但其在首条排序准确性（Hit@1 = 0.70）上相对较低。
...
\begin{itemize}
    ...
    \item \textbf{长文本建模能力：} bge-m3 支持更长上下文输入，能够较好适应工程认证文档中跨段落与复杂结构的语义匹配需求。
...
然而，Bi-Encoder 的高效率建立在“查询与文档独立表示”的机制之上，这也导致其在细粒度语义判别方面存在一定局限。首先，整体语义相似并不等同于能够构成有效证据支撑。在工程认证场景中，用户问题往往涉及“课程—毕业要求支撑关系”等明确逻辑约束，仅依赖向量相似度难以判断文本是否真正回答问题。

其次，文档切块过程可能带来上下文割裂。尽管本文采用语义感知切块策略以尽量保持语义完整性，但在长文档与复杂结构场景中，关键证据仍可能分布于不同文本块甚至跨页位置，从而影响单块检索的有效性。

再次，为保证召回覆盖率，初筛阶段通常设置较大的 Top-K 值，这在提升召回率的同时，也会引入更多“语义相关但不直接回答问题”的噪声片段。相关研究表明，在排序任务中，Cross-Encoder 相较于 Bi-Encoder 具有更强的判别能力\cite{Humeau2020Poly-encoders:}。同时，将 BERT 作为 Cross-Encoder 引入重排序阶段，可以显著提升候选文档的排序质量\cite{nogueira2020passagererankingbert}。

综上所述，单阶段稠密检索适合作为候选召回模块，但难以满足工程认证场景对证据精度与语义判别能力的要求。因此，有必要在初筛结果基础上引入精排机制，以实现从“高召回”到“高精度”的检索优化。
...
在初筛阶段，系统采用 bge-m3 模型对查询与文档进行向量化表示，并基于向量数据库执行相似度检索。根据 M3-Embedding 的研究，bge-m3 模型具备多语言、多粒度与多功能统一建模能力，能够支持较长文本输入，并在检索任务中表现出较强的语义表达能力\cite{chen-etal-2024-m3}。因此，该模型适合作为工程认证场景中的第一阶段召回模型，其核心目标在于最大程度保留潜在相关证据。
...
需要说明的是，本文检索机制完全基于稠密语义匹配与 Cross-Encoder 重排序构建，并未引入 BM25 等稀疏检索方法。这一设计是基于工程认证文档中存在大量同义表达与隐式语义关系的特点，单纯依赖关键词匹配难以有效捕获复杂语义关联。
...
\begin{quote}
你是一个工程教育认证知识助手。请严格依据提供的参考材料回答问题。  
要求：  
...
本文所提出的工程教育认证私有知识助手系统基于 RAG（Retrieval-Augmented Generation）框架实现，整体采用前后端分离架构。系统在开发与部署过程中所使用的关键软硬件环境如下：
...
\begin{itemize}
    ...
    \item \textbf{运行环境：} 系统部署于本地服务器环境，支持 GPU 加速（如 NVIDIA RTX 系列），操作系统为 Windows / macOS。
\end{itemize}

在系统部署流程上，首先完成工程认证相关文档的批量导入，包括培养方案、课程大纲及达成度分析报告等。随后执行文档解析与语义切块操作，将文本块及其元数据（文件路径、页码信息）写入向量数据库。在完成知识库构建后，系统即可对外提供检索与问答服务。
...
本文系统围绕工程认证知识查询需求，设计并实现了多模块协同的功能体系，主要包括文档处理模块、检索模块、生成模块及前端展示模块。
...
因此，在工程教育认证场景中，S3（完整系统）能够更好地满足实际应用对可靠性与可解释性的要求。

\section{总结与展望}
\subsection{全文工作总结}

本文围绕工程教育专业认证（EAC）与成果导向教育（OBE）背景下复杂文档知识管理与查询困难的问题，设计并实现了一个基于检索增强生成（RAG）框架的工程认证私有知识助手系统。

针对工程认证文档类型多样、结构复杂、证据链分散等特点，本文从文档解析、语义检索与生成约束三个层面展开研究，构建了完整的系统技术方案。主要工作总结如下：

（1）在数据处理层，针对工程认证文档结构复杂的问题，提出基于语义感知切块（Semantic Chunking）的文档处理方法，并为每个文本块绑定文件路径与页码等元数据，实现从原始文档到可检索知识单元的结构化转化。
...
（5）在实验评估方面，本文通过端到端实验验证了系统的有效性。实验结果表明，引入重排序机制能够提升检索排序质量，而提示工程约束显著增强了生成结果的证据一致性与可追溯性，从而使系统在工程认证场景中具备良好的应用价值。

综上，本文提出的工程认证私有知识助手系统在检索效率、生成质量与证据链可解释性之间取得了较好的平衡，为复杂工程文档的智能化管理与应用提供了一种可行方案。

\subsection{系统局限性与未来展望}

尽管本文所提出的系统在工程认证知识问答任务中取得了一定效果，但仍存在一些局限性，有待进一步改进。
...
虽然本文采用的 \texttt{bge-m3} 模型在通用语义检索任务中表现良好，但其未针对工程认证领域进行专门训练，在处理专业术语与隐式语义关系时仍存在一定局限。未来可通过领域数据微调或构建领域专用嵌入模型，进一步提升检索效果。
...
综上，本文工作为工程认证文档智能化处理提供了初步探索，未来仍有较大提升空间，有望在更广泛的教育与知识管理场景中发挥作用。
